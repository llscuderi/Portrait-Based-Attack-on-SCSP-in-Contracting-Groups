...
ConjugatorPortrait:=function( g_list, h_list, key_length )
	local G, portrait, depth, PermGroups, AreNotConjugate, ConjugatorPortraitRecursive;
	...
	AreNotConjugateLocal:=function(a,b)
		local l, Glev;
	...
	# TODO: Pass these separately so we know where we are when we need to find leaves
	depth := MaxPortraitDepth(G, key_length, 2) + NucleusDistinctLevel(G);

	portrait := ConjugatorPortraitRecursive( g_list, h_list, depth);
